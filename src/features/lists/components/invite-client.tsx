      ...
      <div className="w-full max-w-md rounded-2xl border border-theme-border bg-theme-surface p-8 text-center shadow-xl">
        {(state.status === "loading" || state.status === "joining") && (
          <>
            ...
            <p className="mt-2 text-sm text-theme-text-muted">Bitte warte einen Moment.</p>
          </>
        )}
        ...
        {state.status === "success" && (
          <>
            ...
            <h1 className="mt-4 text-xl font-semibold text-theme-text">Erfolgreich beigetreten!</h1>
            <p className="mt-2 text-sm text-theme-text-muted">
              Du bist jetzt Mitglied der Liste <strong>{state.listName}</strong>.
            </p>
            <p className="mt-4 text-xs text-theme-text-muted">Du wirst gleich weitergeleitet…</p>
          </>
        )}
        ...
        {state.status === "already-member" && (
          <>
            ...
            <h1 className="mt-4 text-xl font-semibold text-theme-text">Bereits Mitglied</h1>
            <p className="mt-2 text-sm text-theme-text-muted">
        ...
        {state.status === "error" && (
          <>
            ...
            <h1 className="mt-4 text-xl font-semibold text-theme-text">Fehler</h1>
            <p className="mt-2 text-sm text-theme-text-muted">{state.message}</p>
            <button
              onClick={() => router.push("/")}
